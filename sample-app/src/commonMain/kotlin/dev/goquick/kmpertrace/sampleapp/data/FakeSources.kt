...
class FakeNetworkService {
    ...
    suspend fun fetchProfile(userId: String): Profile = traceSpan(component = "NetworkService", operation = "fetchProfile", attributes = mapOf("userId" to userId)) {
    ...
    suspend fun fetchContacts(userId: String): List<Contact> = traceSpan(component = "NetworkService", operation = "fetchContacts", attributes = mapOf("userId" to userId)) {
    ...
    suspend fun fetchActivity(userId: String): List<ActivityEvent> = traceSpan(component = "NetworkService", operation = "fetchActivity", attributes = mapOf("userId" to userId)) {
...
class FakeDatabase {
    ...
    suspend fun loadProfile(userId: String): Profile? = traceSpan(component = "Database", operation = "loadProfile", attributes = mapOf("userId" to userId)) {
        delay(40)
    ...
    suspend fun saveProfile(profile: Profile) = traceSpan(component = "Database", operation = "saveProfile", attributes = mapOf("userId" to profile.id)) {
        delay(30)
        log.withOperation("saveProfile").d { "Saved profile cache for ${profile.id}" }
        cachedProfile = profile
    }

    suspend fun loadContacts(userId: String): List<Contact>? = traceSpan(component = "Database", operation = "loadContacts", attributes = mapOf("userId" to userId)) {
        delay(30)
    ...
    suspend fun saveContacts(userId: String, contacts: List<Contact>) = traceSpan(component = "Database", operation = "saveContacts", attributes = mapOf("userId" to userId)) {
        delay(25)
        log.withOperation("saveContacts").d { "Saved ${contacts.size} contacts for $userId" }
        cachedContacts = contacts
    }

    suspend fun loadActivity(userId: String): List<ActivityEvent>? = traceSpan(component = "Database", operation = "loadActivity", attributes = mapOf("userId" to userId)) {
        delay(20)
    ...
    suspend fun saveActivity(userId: String, events: List<ActivityEvent>) = traceSpan(component = "Database", operation = "saveActivity", attributes = mapOf("userId" to userId)) {
        delay(20)

...
- Raw logs (opt-in): `--raw-logs` or `[r]` in `tui` merges non-KmperTrace lines from the same
  process into the untraced timeline; `[r]` cycles off → debug → info → error → all.
- Parsing in both `print` and `tui`:
    ...
    - When raw is enabled, non-KmperTrace lines from the same process are also shown. We understand
      common Android inputs (`adb logcat -v epoch`, threadtime, Android Studio console copy/paste)
      and Apple unified logs (`log stream/show`), so you can pipe saved files or live streams
      directly into `print` or `tui`.
...
- `?` - help
- `c` - clear
- `r` - cycle raw logs (off → debug → info → error → all)
- `l` - cycle structured min level (all → debug → info → error)

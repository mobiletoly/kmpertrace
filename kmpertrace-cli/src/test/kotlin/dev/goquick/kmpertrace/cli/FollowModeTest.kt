...
class FollowModeTest {
    ...
    fun streaming_input_emits_span_end_with_stack_trace() {
        ...
        val chunked = listOf(
            """prefix |{ ts=2025-01-01T00:00:00Z lvl=info log=Api trace=trace-1 span=s1 parent_span=- ev=SPAN_START name="root" dur=0 head="start" }|""",
            """prefix |{ ts=2025-01-01T00:00:01Z lvl=error log=Api trace=trace-1 span=s1 parent_span=- ev=SPAN_END name="root" dur=10 head="boom" status="ERROR" stack_trace="java.lang.IllegalStateException: boom""",
            """    at A.foo(A.kt:10)""",
            """    at B.bar(B.kt:20)""",
            """ " }|"""
        )
        ...
        val events = ArrayDeque<dev.goquick.kmpertrace.parse.ParsedEvent>()
        ...
        while (true) {
            ...
            if (lastOpen != -1 && lastClose != -1 && lastClose > lastOpen) {
                ...
                parseLine(candidate)?.let { events.add(it) }
        ...
        val end = events.firstOrNull { it.eventKind == dev.goquick.kmpertrace.parse.EventKind.SPAN_END }
        assertTrue(end != null)

...
class RenderGoldenTest {
    ...
    fun renders_error_with_stack_trace_and_attrs() {
        val lines = sequenceOf(
            """t INFO Api |{ ts=2025-01-01T00:00:00Z lvl=info log=Api trace=trace-err span=root parent_span=- ev=SPAN_START name="root" dur=0 head="start" }|""",
            """t ERROR Api |{ ts=2025-01-01T00:00:01Z lvl=error log=Api trace=trace-err span=root parent_span=- ev=SPAN_END name="root" dur=20 head="boom" status="ERROR" error_type="IllegalStateException" error_message="boom" custom="x" stack_trace="java.lang.IllegalStateException: boom\n    at A.foo(A.kt:10)\n    at B.bar(B.kt:20)\n" }|"""
        )
    ...
    fun wraps_without_mid_word_split() {
        val lines = sequenceOf(
            """t INFO Api |{ ts=2025-01-01T00:00:00Z lvl=info log=Api trace=trace-1 span=span parent_span=- ev=LOG name="span" dur=0 head="Download DownloadA starting (jobId=12345678901234567890)" }|"""
        )
    ...
    fun uses_fallback_width_when_auto_unspecified() {
        val lines = sequenceOf(
            """t INFO Api |{ ts=2025-01-01T00:00:00Z lvl=info log=Api trace=trace-2 span=span parent_span=- ev=LOG name="span" dur=0 head="word1 word2 word3 word4 word5 word6" }|"""
        )
    ...
    fun prefers_moving_word_to_next_line_rather_than_splitting() {
        val lines = sequenceOf(
            """t INFO Api |{ ts=2025-01-01T00:00:00Z lvl=info log=Api trace=trace-3 span=span parent_span=- ev=LOG name="span" dur=0 head="Download DownloadA progress 66% (jobId=4453147290101272305)" }|"""
        )
    ...
    fun still_breaks_very_long_words_beyond_budget() {
        ...
        val lines = sequenceOf(
            """t INFO Api |{ ts=2025-01-01T00:00:00Z lvl=info log=Api trace=trace-4 span=span parent_span=- ev=LOG name="span" dur=0 head="$longWord" }|"""
        )
